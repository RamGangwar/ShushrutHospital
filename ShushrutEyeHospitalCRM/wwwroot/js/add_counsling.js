...
var val = $('#frmAddCounseling').validate({
    rules: {
        txtFirstName: {
            required: true,
        },
...
$('#btnReset').on('click', function () {
    $('#frmAddCounseling').validate().resetForm();
});
$(function () {
...
function CreateReception() {
    ...
    var receptionViewModel = {};
    receptionViewModel.Address = $('#txtAddress').val();
    receptionViewModel.ProfilePic = $('#previewProfile').attr('src');
    receptionViewModel.DOB = $('#txtDOB').val()
    receptionViewModel.Gender = $("input[name='gender']:checked").val();
    receptionViewModel.Status = $("input[name='status']:checked").val() == 1 ? true : false;
    var user = {};
    ...
    receptionViewModel.ApplicationUser = user;
    $.ajax({
        type: "POST",
        dataType: "json",
        data: JSON.stringify(receptionViewModel),
        url: GetRoute("/Counsling/AddCounsling"),
        processData: false,
        contentType: "application/json",
        success: function (data) {
            if (data.status == true) {
                swal({
                    title: "Counsling Created",
                    type: "success",
                    text: data.message,
                    confirmButtonColor: "#00B4B4",
                });
                $('#frmAddCounseling').trigger("reset");
                $('#previewProfile').attr('src', '');
            }
            else {
                swal({
                    title: "Counsling Not Created",
                    type: "warning",
                    text: data.message,
                    confirmButtonColor: "#00B4B4",
                });
        ...
        error: function (error) {
            console.log(error);
            swal({
                title: "Counsling creating failed",
                type: "warning",
